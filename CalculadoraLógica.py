...
def CriaTabela(nome_tabela, col = 0): 
    ...
    if col == 3:
        df = pd.DataFrame({'A': [], 'B': [], 'C': []}) # Cria uma tabela com coluna A B C
        Tabela = InserirValores(df, nome_table, col = 3) # Passa a tabela e um parametro com o valor lógico desejado e recebe uma tabela pronta
...
def CriaTabelaDin(nome_tabela, operacao ,col = 0): 
    if col == 3:
        Tabela = InserirValoresDin(nome_tabela, operacao, col = 3) # Passa a tabela e um parametro com o valor lógico desejado e recebe uma tabela pronta
        print(tabulate(Tabela, headers='keys', tablefmt='rounded_grid', showindex= False))
    elif col == 2: 
...
def InserirValores(tabela, operacao, col = 0):
    if col == 2:
        ...
        if operacao == 'V':
            valor = []
            for i in range(cont) : # Realiza verificação dos indices da tabela e realiza a operação guardando o resultado em uma tupla
            ...
            TabelaFormatada = TabelaFinal.astype(int) # Foi necessário usar astype pois a tabela estava com numeros float nas colunas pré definidas
